...
const getStatusInfo = (attraction: Attraction) => {
  const hours = attraction.openingHours;
  if (!hours) return { text: attraction.status || 'Brak danych', isOpen: false };
  ...
  return { text: attraction.status || 'Brak danych', isOpen: false };
};
...
const updateMarkers = () => {
  ...
  const validAttractions = props.attractions.filter(
    a => a.geometry?.latitude !== undefined && a.geometry?.longitude !== undefined
  );

  validAttractions.forEach(attraction => {
    const lat = attraction.geometry!.latitude;
    const lng = attraction.geometry!.longitude;
    const statusInfo = getStatusInfo(attraction);
      ...
      <div class="min-w-[280px]">
        <div class="flex gap-3 p-2">
          ...
          <div class="flex-1 min-w-0">
            <h3 class="text-base font-bold text-gray-900 mb-1 cursor-pointer hover:text-blue-600 transition-colors attraction-name-link leading-tight" data-attraction-id="${attraction.id}">
              ${attraction.name}
            </h3>
            
            <div class="flex items-center gap-2 mb-2">
              <span class="px-2 py-0.5 text-xs font-semibold rounded ${
                statusInfo.isOpen ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
              }">
                ${statusInfo.text}
              </span>
              ${attraction.openingHours ? `<span class="text-xs text-gray-600">${attraction.openingHours}</span>` : ''}
            </div>
            ...
            <div class="flex items-center gap-2 flex-wrap text-xs">
              ${
                ...
                <div class="flex items-center bg-yellow-50 px-1.5 py-0.5 rounded">
                  ...
                  <span class="ml-1 font-bold text-gray-900">${attraction.rating.toFixed(1)}</span>
                </div>
              `
                  : ''
              }
              ...
              ${
                attraction.category
                  ? `
                ${attraction.rating || attraction.reviews ? '<span class="text-gray-300">•</span>' : ''}
                <span class="font-medium text-gray-700">${attraction.category}</span>
              `
                  : ''
              }
            </div>
      ...
      setTimeout(() => {
        ...
        if (popup) {
          const nameLink = popup.querySelector(
            `.attraction-name-link[data-attraction-id="${attraction.id}"]`
          );
          if (nameLink) {
  ...
  if (validAttractions.length > 0) {
    const bounds = L.latLngBounds(
      validAttractions.map(a => [a.geometry!.latitude, a.geometry!.longitude] as [number, number])
    );
